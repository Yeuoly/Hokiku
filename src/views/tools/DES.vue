<template>
  <div>
    <v-container>
      <v-row>
        <v-col :cols="12">
          <v-alert outlined type="success" text> DES 加密 </v-alert>
          <v-divider></v-divider>
          <v-card-text>
...
export default {
  ...
  methods: {
    ...
    encrypt() {
      ...
      const ivBytes =
        this.selectedMode !== "ECB" ? enc.Utf8.parse(this.iv) : null;
      const srcBytes = enc.Utf8.parse(this.src);
      let cipherText;

      if (this.selectedMode === "ECB") {
        cipherText = DES.encrypt(srcBytes, keyBytes, {
    ...
    decrypt() {
      ...
      if (this.selectedMode === "ECB") {
        decryptedText = DES.decrypt(this.src, keyBytes, {
          mode: CryptoJs.mode.ECB,
          padding: CryptoJs.pad.Pkcs7,
        });
      } else {
