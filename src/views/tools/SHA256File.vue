          ...
          <v-col :cols="12">
            <v-card-text>
              <v-file-input
                ...
                label="选择文件"
                outlined
                placeholder="选择需要计算 SHA256 哈希值的文件"
              ></v-file-input>
            </v-card-text>
            <v-divider></v-divider>
          </v-col>
          <v-col :cols="12">
            <v-card-text>
              <v-textarea
                ...
                outlined
                rows="1"
                placeholder="计算得到的 SHA256 哈希值将显示在此处"
              ></v-textarea>
            </v-card-text>
            <v-divider></v-divider>
          </v-col>
...
function calcFileHash(file, callback) {
  let blobSlice = File.prototype.slice || File.prototype.mozSlice || File.prototype.webkitSlice,
    ...
    currentChunk = 0,
    hasher_sha256 = CryptoJS.algo.SHA256.create();

  let fileReader = new FileReader();

    fileReader.onload = function (e) {
    let CryptoJS_data = CryptoJS.enc.Latin1.parse(e.target.result);
    hasher_sha256.update(CryptoJS_data);

    currentChunk += 1;
    if (currentChunk < chunks) {
      loadNext();
    } else {
      callback(hasher_sha256.finalize().toString(CryptoJS.enc.Hex));
    }
  };
