        ...
        <v-row class="px2 py2">
            ...
            <v-col :cols="12">
                <v-card>
                    <v-card-text>
                        <v-row
                            v-for="(i, k) in rank_user"
                            :key="k"
                        >
                            <v-col cols="12">
                                <div>
                                    <v-icon v-if="k == 0" color="primary">
                                        mdi-license
                                    </v-icon>
                                    <v-icon v-else-if="k == 1" color="green">
                                        mdi-trophy-outline
                                    </v-icon>
                                    <v-icon v-else-if="k == 2" color="red">
                                        mdi-trophy-award
                                    </v-icon>
                                    <v-icon v-else>
                                        mdi-trophy-variant
                            ...
                            <v-col 
                                ...
                                :lg="2"
                                :xl="2"
                                v-for="(j, l) in i.trains"
                                :key="l"
                            >
                                <v-chip color="green" class="px3 py3" dark>
                                    <v-icon>
                            ...
                            <v-col cols="12">
                                ...
                                >
                                    <span class="white--text">
                                        {{ (i.score / max_score * 100).toFixed(2) }}%
                                    </span>
                                </v-progress-linear>
                            </v-col>
                        </v-row>
...
export default {
    ...
    computed : {
        rank_user() {
            ...
            const calc_score = (origin_score, solved) => {
                const min_score = parseInt(origin_score * 0.1)
                const max_score = origin_score
                if (solved == 0) {
                    return max_score
                }
                const score = 1.000 / parseFloat(solved) * (max_score - min_score) + min_score
                return score
            }

            for (let i = 0; i < this.rank.length; i++) {
                const u = find_user(this.rank[i].uid)
                if (!u) {
                    user.push({
                        uid : this.rank[i].uid,
                        username : this.rank[i].username,
                        solved : 0,
                        score : calc_score(500, this.rank[i].train_solves),
                        trains : [{
                            id : this.rank[i].train_id,
                            name : this.rank[i].train_name,
                            score : calc_score(500, this.rank[i].train_solves)
                        }]
                    })
                } else {
                    u.score += calc_score(500, this.rank[i].train_solves)
                    u.trains.push({
                        id : this.rank[i].train_id,
                        name : this.rank[i].train_name,
                        score : calc_score(500, this.rank[i].train_solves)
        ...
        max_score() {
            let max = 0
            for (let i = 0; i < this.rank_user.length; i++) {
                if (this.rank_user[i].score > max) {
                    max = this.rank_user[i].score
                }
            }
            return max
        }
    ...
    methods : {
        ...
        async load(){
            ...
            const { data } = await api_organization_train_sheet_detail(this.gid, this.sid)
            if(data && data['res'] == 0){
                this.trains = data['data']['list']
                this.sheet_name = data['data']['sheet']['name']
                this.sheet_comment = data['data']['sheet']['comment']
            }else {
                openErrorMessageBox('错误', data ? data['err'] : '未知错误')
            }
        },
